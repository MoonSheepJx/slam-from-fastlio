...
typedef vector<PointNomalType, Eigen::aligned_allocator<PointNomalType>>  PointVector;
...
struct StateGroup
{
    ...
    Eigen::Matrix<double, DIM_STATE, DIM_STATE> cov;      //状态的协方差
    Eigen::Vector3d T_L_I;
    Eigen::Matrix3d R_L_I;
    double last_update_time = 0;
    StateGroup();
    ~StateGroup();
    StateGroup operator+(const Eigen::Matrix<double, DIM_STATE, 1> &state_add)
    {
        StateGroup a;
        ...
        a.gyr_bias  = this->gyr_bias  + state_add.block<3,1>(9,0);
        a.acc_bias  = this->acc_bias  + state_add.block<3,1>(12,0);
        a.gravity = this->gravity + state_add.block<3,1>(15,0);
        a.cov     = this->cov;
	    return a;
    };
    Eigen::Matrix<double, DIM_STATE, 1> operator-(const StateGroup& b)
	{
        ...
        a.block<3,1>(9,0)  = this->gyr_bias  - b.gyr_bias;
        a.block<3,1>(12,0) = this->acc_bias  - b.acc_bias;
...
StateGroup::StateGroup()                    //构造函数
{
    ...
    T_L_I = Eigen::Vector3d::Zero();
    R_L_I = Eigen::Matrix3d::Identity();
}
